			...
			case '+':
				...
				if (phase == 0) {
					...
					if (strstr(line, "+ MNM Gameserver") != 0) {
						...
						if(shm->eigspielernummer<1){
							
							printf("spielernummer ist kleiner 1");
							return EXIT_FAILURE;
						...
						if(shm->anzahlspieler < 1){
							...
							printf("anzahlspieler ist kleiner 1");
							return EXIT_FAILURE;
						
						}						
						else if(shm->anzahlspieler > 8){
						
							printf("Die Anzahl der Spieler ist größer als 8");
							return EXIT_FAILURE;
